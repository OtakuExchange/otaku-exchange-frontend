    ...
    <Box sx={{ width: 220, flexShrink: 0, borderRight: 1, borderColor: 'divider', minHeight: '100%' }}>
      <List dense disablePadding>
        ...
        <Collapse in={seedOpen} timeout="auto" unmountOnExit>
          <List dense disablePadding>
            <ListItemButton sx={{ pl: 4 }} onClick={() => navigate('/admin/seed/market')}>
...
function SeedMarketView() {
  ...
  return (
    <Box sx={{ maxWidth: 480 }}>
      <Typography variant="h6" gutterBottom>Seed Market</Typography>
      <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
        Places YES and NO limit orders around a midpoint price to provide initial liquidity.
      </Typography>
      <TextField
        label="Market ID"
        value={marketId}
...
function AdminGuard({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    console.log('[AdminGuard] isLoaded:', isLoaded, '| isSignedIn:', isSignedIn)
    if (!isLoaded) return
    if (!isSignedIn) {
