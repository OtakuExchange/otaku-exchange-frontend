...
function statusColor(status: string): string {
  switch (status.toLowerCase()) {
    case "open": return "#1b5e20";
    case "staking_closed": return "#e65100";
    case "hidden": return "#424242";
...
function statusLabel(status: string): string {
  switch (status.toLowerCase()) {
    case "staking_closed": return "Staking Closed";
...
export default function EventStatusView() {
  ...
  const [eventsByTopic, setEventsByTopic] = useState<{ topicId: UUID; topicLabel: string; events: Event[] }[]>([]);
  ...
  useEffect(() => {
    ...
    setLoading(true);
    Promise.all(topics.map((t) => fetchEvents(t.id).then((evts) => ({ topic: t, evts }))))
      .then((results) => {
  ...
  return (
    <Box sx={{ maxWidth: 640 }}>
      <Typography variant="h5" gutterBottom>Event Visibility</Typography>
      <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
        Toggle events between open, staking closed, and hidden.
      </Typography>
      {error && <Alert severity="error" sx={{ mb: 2 }}>{error}</Alert>}
      {eventsByTopic.length === 0 ? (
        <Typography color="text.secondary">No open, staking closed, or hidden events found.</Typography>
      ) : (
        eventsByTopic.map((group) => (
          <Box key={group.topicId} sx={{ mb: 3 }}>
            <Typography
              variant="subtitle1"
              fontWeight={700}
              sx={{ mb: 1, px: 1, color: "#7B8996", textTransform: "uppercase", fontSize: "12px", letterSpacing: 1 }}
            >
              {group.topicLabel}
            </Typography>
            {group.events.map((event, i) => (
              <Box key={event.id}>
                ...
                >
                  <Box sx={{ minWidth: 0 }}>
                    <Typography fontWeight={600} noWrap>{event.name}</Typography>
                    <Chip
                  ...
                  <Stack direction="row" spacing={1} sx={{ flexShrink: 0 }}>
                    {STATUS_OPTIONS.map((s) => (
                      <Button
                        key={s}
                        size="small"
                        variant={event.status.toLowerCase() === s ? "contained" : "outlined"}
                        onClick={() => handleToggle(event, s)}
                        disabled={togglingId === event.id || event.status.toLowerCase() === s}
                        startIcon={
                          togglingId === event.id && event.status.toLowerCase() !== s
